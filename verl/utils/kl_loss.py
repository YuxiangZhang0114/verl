...
class _VocabParallelKLDivergence(torch.autograd.Function):
    ...
    def forward(ctx, vocab_parallel_logits, target_topk_logps, target_topk_indices):
        if not _MEGATRON_AVAILABLE:
            raise ImportError("Megatron-LM is not available")
...
def fsdp_kl_divergence(logits, target_topk_logps, target_topk_indices):
    """
    Compute KL divergence for FSDP (no tensor parallelism).
    
    Args:
        logits: [batch_size, seq_len, vocab_size]
        target_topk_logps: [batch_size, seq_len, k]
        target_topk_indices: [batch_size, seq_len, k]
        
    Returns:
        kl_loss: [batch_size, seq_len]
    """
    # Calculate log_probs from logits
    # log_probs = F.log_softmax(logits, dim=-1) # [batch_size, seq_len, vocab_size]
    
    # We need log_probs at target_topk_indices
    # Gather log_probs
    # gathered_log_probs = torch.gather(log_probs, dim=-1, index=target_topk_indices)
    
    # Memory efficient implementation:
    # 1. Calculate log_softmax
    log_probs = F.log_softmax(logits, dim=-1)
    
    # 2. Gather only what we need
    gathered_log_probs = torch.gather(log_probs, dim=-1, index=target_topk_indices)
    
    # 3. Calculate target probabilities
    target_probs = torch.exp(target_topk_logps)
    
    # 4. KL(P || Q) = sum(P(x) * (log P(x) - log Q(x)))
    kl_loss = torch.sum(target_probs * (target_topk_logps - gathered_log_probs), dim=-1)
