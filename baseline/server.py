...
class LRUCache:
    ...
    def __init__(self, max_size: int = 200000):
        ...
        Args:
            max_size: Maximum number of cached entries (default: 200,000)
        """
    ...
    def put(self, key: str, value: Any):
        """Put value into cache, evict oldest if full."""
        with self.lock:
            if key in self.cache:
                # Update existing key
                self.cache.move_to_end(key)
...
class Encoder:
    ...
    def __init__(self, model_name: str, model_path: str, pooling_method: str, max_length: int, use_fp16: bool, gpu_id: int = 0):
        ...
        # Add lock for concurrent GPU access control
    ...
    def encode(self, query_list: list[str], is_query: bool = True) -> np.ndarray:
        """Encode queries or passages into embeddings with concurrent GPU access control."""
        # Use lock to serialize GPU encoding operations
        # This prevents OOM errors when multiple requests hit the GPU simultaneously
...
class Config:
    ...
    def __init__(self, retrieval_method: str = "e5", retrieval_topk: int = 10, index_path: str = "./index/e5_Flat.index", corpus_path: str = "./data/corpus.jsonl", faiss_gpu: bool = True, gpu_id: int = 0, retrieval_model_path: str = "intfloat/e5-base-v2", retrieval_pooling_method: str = "mean", retrieval_query_max_length: int = 256, retrieval_use_fp16: bool = True, retrieval_batch_size: int = 512, cache_size: int = 200000, enable_cache: bool = True):
...
class DenseRetriever(BaseRetriever):
    ...
    def __init__(self, config: Config):
        ...
        # Add lock for FAISS GPU search operations
        ...
        if self.enable_cache:
            ...
            logger.info(f"Query cache enabled with max size: {config.cache_size}")
        else:
            self.cache = None
        ...
        if config.faiss_gpu:
            try:
                ...
                if num_gpus > 0:
                    # Create GPU resources with larger temporary memory
                    # For large indices (61GB Flat index), we need more temp memory
                    res = faiss.StandardGpuResources()
                    # Set temp memory to 60GB (needs to be larger than index size)
                    res.setTempMemory(60 * 1024 * 1024 * 1024)  # 60 GB
                    
                    # Convert to GPU using single GPU with custom resources
                    # For very large indices, use single GPU instead of multi-GPU
                    logger.info(f"Attempting to move FAISS index to GPU {config.gpu_id}...")
                    self.index = faiss.index_cpu_to_gpu(res, config.gpu_id, self.index)
                    self.faiss_on_gpu = True
                    logger.info(f"✓ FAISS index successfully moved to GPU {config.gpu_id} with 60GB temp memory")
                else:
                ...
                logger.info("Continuing with CPU index (slower but functional)")
    ...
    def _search(self, query: str, num: int = None, return_score: bool = False):
        ...
        # Check cache first
        cache_key = f"{query}||{num}"  # Use || as separator to avoid conflicts
        if self.enable_cache:
            cached_result = self.cache.get(cache_key)
            if cached_result is not None:
                # Cache hit
                if return_score:
                    return cached_result["results"], cached_result["scores"]
                else:
        ...
        # Lock FAISS search if index is on GPU to prevent concurrent memory issues
        ...
        # Store in cache
        if self.enable_cache:
            self.cache.put(cache_key, {"results": results, "scores": scores_list})
...
def health_check():
    ...
    if config.enable_cache and hasattr(retriever, "cache") and retriever.cache is not None:
        response["cache_stats"] = retriever.cache.stats()
...
def main():
    ...
    parser.add_argument("--max_length", type=int, default=128, help="Maximum query length for tokenization")
    ...
    parser.add_argument("--cache_size", type=int, default=200000, help="Maximum number of cached queries (default: 200,000)")
    parser.add_argument("--no_cache", action="store_true", help="Disable query caching")
    ...
    if not args.no_cache:
        print(f"  Cache Size: {args.cache_size:,} queries")
    print("=" * 60)
