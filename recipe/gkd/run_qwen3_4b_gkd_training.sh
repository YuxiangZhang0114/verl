...
# Model path - Qwen3-4B-Instruct
HF_MODEL_PATH="Qwen/Qwen3-4B-Instruct-2507"

# Data paths - 根据你的实际数据路径修改
...
# Save path
save_path="/mnt/workspace/checkpoints/qwen3_4b_gkd_distill"

# Parallel configuration for Qwen3 4B
NODES=1
PP=1  # Pipeline parallel size (1 for 4B model)
TP=1  # Tensor parallel size (1 for 4B model)
INFER_TP=1  # Inference tensor parallel size
SP=False  # Sequence parallel (only needed when TP > 1)
...
# 使用 ray job submit 提交任务（推荐用于分布式训练）
# 如果只是本地训练，可以直接使用 python3 -m recipe.gkd.main_gkd

python3 -m recipe.gkd.main_gkd \
    --config-path="${WORKING_DIR}/config" \
    ...
    trainer.experiment_name="qwen3-4b-instruct-gkd-$(now)" \
